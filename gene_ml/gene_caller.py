...
def get_gene_ml_events(preds):
    return get_gene_ml_events_unwrapped(preds['cds_start'], preds['cds_end'], preds['exon_start'], preds['exon_end'])
...
def get_gene_ml_events_unwrapped(cds_start: np.ndarray, cds_end: np.ndarray, exon_start: np.ndarray, exon_end: np.ndarray):
    ...
    events.extend([GeneEvent(i, CDS_START, cds_start[i]) for i in cds_starts])
    events.extend([GeneEvent(i, CDS_END, cds_end[i]) for i in cds_ends])
    events.extend([GeneEvent(i, EXON_START, exon_start[i]) for i in exon_starts])
    events.extend([GeneEvent(i, EXON_END, exon_end[i]) for i in exon_ends])
    events.sort()
...
@njit
def get_end_idx(start_idx: int, events: list[GeneEvent], is_exon_pred: np.ndarray, is_intron_pred: np.ndarray) -> int:
    event = events[start_idx]
    start_pos = event.pos
    pos = start_pos
    num_good_bases = 0
    last_good_base = None
    while pos < len(is_exon_pred):
        if is_exon_pred[pos] > 0.1 or is_intron_pred[pos] > 0.1:
            num_good_bases += 1
            last_good_base = pos
        else:
...
@njit
def score_gene_call(preds: dict, gene_call: list[GeneEvent], seq: str, debug=False):
    last_pos = None
    summed_scores = 0
    num_vals = 0
    for pos, event, score in gene_call:
        if last_pos is not None:
            ...
            key = 'is_exon' if is_exon else 'is_intron'
            summed_scores += np.sum(preds[key][last_pos:pos])
            num_vals += len(preds[key][last_pos:pos])
        last_pos = pos
    ...
    for i, e in enumerate(gene_call[:-1]):
        if e.type == EXON_END:
            ...
            intron_score += np.mean(preds['is_intron'][e.pos+1:e2.pos]) - np.mean(preds['is_exon'][e.pos+1:e2.pos])
...
def produce_gene_calls(preds: dict, events: list[GeneEvent], seq: str, time_limit=6, debug=False) -> list[tuple[float, list[GeneEvent]]]:
    ...
    for start_idx in range(len(events)):
        ...
        if event.type == CDS_START:
            end_idx = get_end_idx(start_idx, events, preds['is_exon'], preds['is_intron'])
...
def build_gene_calls(preds: dict, rc_preds: dict, seq: str, rc_seq: str = None,
    ...
    if debug:
        print('\n******************** forward strand')
    events = get_gene_ml_events(typed.Dict(preds.items()))
    # for event in events:
