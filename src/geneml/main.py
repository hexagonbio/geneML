...
def get_git_version():
    repo = git.Repo(search_parent_directories=True)     #TODO: If hash matches an official release, output release version
...
def main():
    ...
    parser.add_argument('-o', '--output', type=str, nargs='?', help="Gene annotations output path (default: based on input filename).")
    parser.add_argument('-g', '--genes', type=str, nargs='?', help="Gene sequences output path (default: None).")
    parser.add_argument('-p', '--proteins', type=str, nargs='?', help="Protein sequences output path (default: None).")
    parser.add_argument('-m', '--model', type=str, nargs='?', help="Model ID or path to model file.")
    parser.add_argument('-c', '--cores', type=int, nargs='?', help="Number of cores to use for processing (default: all available).")
